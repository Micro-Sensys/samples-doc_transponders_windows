...
def ModifyConnectionSettings(hostname):
    while True:
        ...
        if selValue == "1":
            print("\t1 - GET possible settings")
            answer = PerformGetRequest('http://' + hostname + ':19813/api/iidservice/doc/interface/PossibleSettings')
            if answer != None:
            ...
            print("\t2 - GET current settings")
            answer = PerformGetRequest('http://' + hostname + ':19813/api/iidservice/doc/interface/CurrentSettings')
            if answer != None:
                if answer.status_code == 200:
                    print(answer.text)
                else:
            ...
            if portType != "4":
                ...
                portName = ""
            print("InterfaceType: ", end="")
            interfaceType = input()
            answer = PerformPostRequest('http://' + hostname + ':19813/api/iidservice/doc/interface/CurrentSettings?&PortName='+portName+'&PortType='+portType+'&InterfaceType='+interfaceType)
            if answer != None:
                if answer.status_code == 200:
                    print("\t\t=> OK")
                else:
