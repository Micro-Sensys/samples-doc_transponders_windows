...
namespace SampleAsync_CSharp
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private async void Button_Identify_ClickAsync(object sender, RoutedEventArgs e)
        {
            ...
            if (m_DocInterface != null)
            {
                if (m_DocInterface.IsInitialized)
                {
                    ...
                    try
                    {
                        ...
                        var identifyResult = await m_DocInterface.IdentifyAsync();
        ...
        private async void Button_ReadBytes_ClickAsync(object sender, RoutedEventArgs e)
        {
            ...
            if (m_DocInterface != null)
            {
                if (m_DocInterface.IsInitialized)
                {
                    ...
                    try
                    {
                        textBox_ThreadLog.Text += "\n = ReadBytesAsync =\n";
                        textBox_ThreadLog.ScrollToEnd();
                        var result = await m_DocInterface.ReadBytesAsync(tagID, page, from, length);
                        TimeSpan processSpan = DateTime.UtcNow - startTime;
                        if (result != null)
                        {
                            //Update result in UI
        ...
        private async void Button_WriteBytes_ClickAsync(object sender, RoutedEventArgs e)
        {
            if (comboBox_TagID.SelectedIndex == -1)
            {
            ...
            if (m_DocInterface != null)
            {
                if (m_DocInterface.IsInitialized)
                {
                    ...
                    try
                    {
                        textBox_ThreadLog.Text += "\n = WriteBytesAsync =\n";
                        textBox_ThreadLog.ScrollToEnd();
                        bool result = await m_DocInterface.WriteBytesAsync(tagID, page, from, dataToWrite, false);
                        TimeSpan processSpan = DateTime.UtcNow - startTime;
