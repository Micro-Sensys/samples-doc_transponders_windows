...
DocInterfaceControl Console_InitializeDocInterfaceControl()
{
    ...
    Console.WriteLine("");
    DocInterfaceControl result = new(readerPortSettings, interfaceType);
...
void DocInterfaceControl_InitializeCompleted(object _sender, bool _portOpen)
{
...
int Console_InitializeInterfaceType()
{
...
bool docOperationCompleted = false;
void DocInterfaceControl_DocResultChanged(object sender, DocResultEventArgs _docResult)
{
    ...
    if (_docResult != null)
    {
        ...
        if (_docResult.ResultInfo != null)
        {
            if (_docResult.ResultInfo is HfScanResultInfo hfResult)
            {
                ...
                Console.WriteLine("\t" + BitConverter.ToString(hfResult.TagID));
...
bool Console_ExecuteAndContinue(DocInterfaceControl _docIntControl)
{
    //Main Console MENU
    Console.WriteLine("");
    Console.WriteLine("--------------------");
...
void Console_Execute_ReadReaderID(DocInterfaceControl _docIntControl)
{
...
void Console_Execute_Identify(DocInterfaceControl _docIntControl)
{
    m_LastTagID = null;
    //First make sure DocInterfaceControl is initialized
    if (_docIntControl != null)
...
void Console_Execute_ReadBytes(DocInterfaceControl _docIntControl)
{
...
void Console_Execute_WriteBytes(DocInterfaceControl _docIntControl)
{
    ...
    if (_docIntControl != null)
    {
        if (_docIntControl.IsInitialized)
        {
            ...
            byte[] toWrite = [0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF];
            docOperationCompleted = false;
            //Start WriteBytes
