...
Module Program
    ...
    Private Function Console_ExecuteAndContinue(docIntControl As DocInterfaceControl) As Boolean
        ...
        Select Case operationNumTxt
            Case "0"
                ...
                Console_Execute_ReadReaderIDAsync(docIntControl)
            Case "1"
                Console.WriteLine(vbTab + "Identfy")
                Console_Exceute_IdentifyAsync(docIntControl)
            Case "2"
                Console.WriteLine(vbTab + "ReadBytes")
                Console_Execute_ReadBytesAsync(docIntControl)
            Case "3"
                Console.WriteLine(vbTab + "WriteBytes")
                Console_Execute_WriteBytesAsync(docIntControl)
            Case "x", "X"
                Return False
        End Select
        Thread.Sleep(500)
    ...
    Private Async Sub Console_Execute_ReadReaderIDAsync(docIntControl As DocInterfaceControl)
    ...
    Private Async Sub Console_Exceute_IdentifyAsync(docIntControl As DocInterfaceControl)
        m_LastTagID = Nothing
        'First makes ure DocInterfaceContorl is initialized
        If docIntControl IsNot Nothing Then
            If docIntControl.IsInitialized Then
                Try
                    ...
                    Dim tagScanInfo = Await docIntControl.IdentifyAsync()
                    Dim processSpan As TimeSpan = Date.UtcNow - startTime
    ...
    Private Async Sub Console_Execute_ReadBytesAsync(docIntControl As DocInterfaceControl)
        ...
        If docIntControl IsNot Nothing Then
            If docIntControl.IsInitialized Then
                Try
                    ...
                    Dim readResult = Await docIntControl.ReadBytesAsync(m_LastTagID, 3, 0, 16)
                    Dim processSpan As TimeSpan = Date.UtcNow - startTime
    ...
    Private Async Sub Console_Execute_WriteBytesAsync(docIntControl As DocInterfaceControl)
        ...
        If docIntControl IsNot Nothing Then
            If docIntControl.IsInitialized Then
                Try
                    ...
                    If Await docIntControl.WriteBytesAsync(m_LastTagID, 3, 0, toWrite, False) Then
